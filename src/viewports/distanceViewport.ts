...
function distanceMapPipelineDescriptor(device: GPUDevice): GPURenderPipelineDescriptor {
    return {
        ...
        vertex: {
            module: device.createShaderModule({ code: distanceShader }),
...
export class DistanceViewport {
    ...
    constructor(
        ...
        positions: Array<vec3> | null = null,
...
/*        console.log("Context:" + this._context);
    ...
    public setPositions(positions: Array<vec3>): void {
        ...
        this.cameraConfiguration = {
            ...
            zoom: 0.5 * (positions.length * 1.4142),
            translateX: (-0.5 * (positions.length * 1.4142)),
            translateY: (-0.5 * (positions.length * 0.7071)),
            maxZoom: (positions.length * 1.4142),
        };
    }
