...
export class Spline extends IParametricObject {
    ...
    static gpuCodeIntersectionTest = /* wgsl */`
        ...
        fn ray${this.typeName}Intersection(ray: Ray, curveWS: ${this.typeName}) -> CurveIntersection {
            ...
            if (!hit) {
                ...
                result.position = ray.origin.xyz + result.t * ray.direction.xyz;
                let curvePosition = evaluateQuadraticBezier(QuadraticBezierCurve(curveWS.p0.xyz, curveWS.p1.xyz, curveWS.p2.xyz, 0), result.curveT);
    ...
    static gpuCodeGetOutputValue(variable: "color" | "normal" | "ao"): string {
        switch (variable) {
            case "color": {
                return `
                    let color = phong(ray, Intersection (
                        intersection.t,
...
export function lineToSpline(points: vec3[]): vec3[] {
    // Pad the spline with invisible start and end point
    const startPoint = vec3.add(
        vec3.create(),
        vec3.sub(vec3.create(), points[0], points[1]),
        points[0]
    );
    ...
    for (let i = 0; i < catmullRomPoints.length - 3; i++) {
        ...
        cubicBezierPoints[i + 1][0] =
            (d1_2alpha * p2[0] -
                d2_2alpha * p0[0] +
                (2.0 * d1_2alpha + 3.0 * d1_alpha * d2_alpha + d2_2alpha) * p1[0]) /
            (3.0 * d1_alpha * (d1_alpha + d2_alpha));
        cubicBezierPoints[i + 1][1] =
            (d1_2alpha * p2[1] -
                d2_2alpha * p0[1] +
                (2.0 * d1_2alpha + 3.0 * d1_alpha * d2_alpha + d2_2alpha) * p1[1]) /
            (3.0 * d1_alpha * (d1_alpha + d2_alpha));
        cubicBezierPoints[i + 1][2] =
            (d1_2alpha * p2[2] -
                d2_2alpha * p0[2] +
                (2.0 * d1_2alpha + 3.0 * d1_alpha * d2_alpha + d2_2alpha) * p1[2]) /
            (3.0 * d1_alpha * (d1_alpha + d2_alpha));

        cubicBezierPoints[i + 2][0] =
            (d3_2alpha * p1[0] -
                d2_2alpha * p3[0] +
                (2.0 * d3_2alpha + 3.0 * d3_alpha * d2_alpha + d2_2alpha) * p2[0]) /
            (3.0 * d3_alpha * (d3_alpha + d2_alpha));
        cubicBezierPoints[i + 2][1] =
            (d3_2alpha * p1[1] -
                d2_2alpha * p3[1] +
                (2.0 * d3_2alpha + 3.0 * d3_alpha * d2_alpha + d2_2alpha) * p2[1]) /
            (3.0 * d3_alpha * (d3_alpha + d2_alpha));
        cubicBezierPoints[i + 2][2] =
            (d3_2alpha * p1[2] -
                d2_2alpha * p3[2] +
                (2.0 * d3_2alpha + 3.0 * d3_alpha * d2_alpha + d2_2alpha) * p2[2]) /
            (3.0 * d3_alpha * (d3_alpha + d2_alpha));
